...
router.get('/judge', auth, async (req, res) => {
    try {
        ...
        if (!judge) {
            return res.render('error', { message: 'Judge profile not found' });
        }
        ...
        console.error('Dashboard error:', error);
        return res.render('error', { message: 'Error loading dashboard', error });
    }
});
...
router.get('/organizer/manage-hackathons', auth, async (req, res) => {
    try {
        ...
        const organizer = await Organizer.findOne({ user: user._id });
        if (!organizer) {
            return res.render('error', { message: 'Organizer profile not found' });
        }
...
router.get('/organizer/hackathons', auth, async (req, res) => {
    try {
        ...
        res.render('dashboard/hackathons', {  // You'll need to create this view
            user,
            organizer,
            hackathons,
            layout: 'dashboard/layout',
...
// Organizer's Participants route
...
// The participants route section
